...
public class CommonDAOImpl extends HibernateDaoSupport implements CommonDAO {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@Override
	public <T> List<T> findByMapArg(final String queryStr, final Map map,
			final List<Order> orders,final int thispage,final int pagesize) {
		return this.getHibernateTemplate().executeFind( new HibernateCallback () {
			 public Object doInHibernate ( Session session ) throws HibernateException, SQLException {
				 String queryString = queryStr;
					...
					for (Order order : orders) {
						if (count < 1) {
							if (order.getDirection().ordinal() == 0) {
								queryString += " order by t." + order.getProperty()
										+ " asc";
							} else {
								queryString += " order by t." + order.getProperty()
										+ " desc";
							...
							if (order.getDirection().ordinal() == 0) {
								queryString += " , t. " + order.getProperty() + " asc";
							} else {
								queryString += " , t. " + order.getProperty() + " desc";
							}
							count++;
						}
					}
	...
	@Override
	public <T> void saveAndUpdateAll(List<T> params) {
		// TODO Auto-generated method stub
		getHibernateTemplate().saveOrUpdateAll(params);
	}
