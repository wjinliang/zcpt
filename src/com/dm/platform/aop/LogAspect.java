...
public class LogAspect {
	private static final Logger logErrorFile = LoggerFactory
			.getLogger("logErrorFile");
	private static final Logger logServiceFile = LoggerFactory
			.getLogger("logServiceFile");
	@Autowired
	...
	public void insertServiceCallCalls(JoinPoint joinPoint, Object rtv)
		...
		log.setContent(adminOptionContent(joinPoint.getArgs(), user.getName()
				+ ":新建"));
		log.setDate(DmDateUtil.DateToStr(new Date(),
				"yyyy-MM-dd HH:mm:ss"));
		log.setType("1");
	...
	public void afterThrowing(JoinPoint joinPoint, RuntimeException e) {
		...
		log.setContent("抛出的异常: " + e.getMessage() + ">>>>>>>" + e.getCause());
		log.setDate(DmDateUtil.DateToStr(new Date(),
				"yyyy-MM-dd HH:mm:ss"));
		log.setType("1");
		log.setIp(UserAccountUtil.getInstance().getCurrentIp());
		commonDAO.save(log);
		logErrorFile.error(user.getName() + "(" + user.getCode() + ")"
	...
	public Object aroundCall(ProceedingJoinPoint pjp) throws Throwable {
		...
		try {
			...
			logServiceFile.info(pjp.getTarget().getClass().getName() + "."
					+ pjp.getSignature().getName() + "耗时：" + procTime + "ms");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private String adminOptionContent(Object[] args, String mName)
		...
		for (Object info : args) {
			...
			for (Method method : methods) {
				...
				// 判断是不是get方法
				if ((methodName.indexOf("getCode") == -1)
						&& (methodName.indexOf("getId") == -1)) {// 不是get方法
					continue;// 不处理
				}
