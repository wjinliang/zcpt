...
public class ResourceMapCache {
	...
	public void refreshResourceMap(){
		...
		String sql = "SELECT  b.`CODE` AS role,g.`MENUURL` AS url FROM t_user_menu g, t_role_menu f,t_user_role b WHERE g.`id`=f.MENU_CODE AND  f.ROLE_CODE=b.`CODE` AND g.`MENUURL`!='/#' ORDER BY role,url"; 
		...
		/*String sql2 = "SELECT m.MENUURL as url from t_user_menu m where m.id not in(SELECT g.id FROM t_user_menu g, t_role_menu f WHERE g.`id` = f.MENU_CODE ) AND m.`MENUURL`!='/#' ORDER BY url";
		JDBCUtil.getJdbcTemplate().query(sql2, new RowCallbackHandler() {  
            ...
            public void processRow(ResultSet rs) throws SQLException {  
                ...
                if(!resourceMap.containsKey(url)){
                	Collection<ConfigAttribute> configAttributes = new ArrayList<ConfigAttribute>();
                	configAttributes.add(new SecurityConfig("superchen"));
                	resourceMap.put(url,configAttributes);
                }
            }  
        });*/ 
