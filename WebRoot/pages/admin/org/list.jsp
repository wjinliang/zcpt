...
<!-- END HEAD -->
	...
	<!-- END HEADER -->
		...
		<div class="page-content">
				...
				<div class="row-fluid">
					<div class="span6">
						<div class="portlet box green" >
							...
							<div class="scroller" data-height="50px">
			...
			<ul class="dropdown-menu dropdown-context">
				<li class="nav-header" id="m_add" onclick="addMenu();">增加子级组织</li>
				<li class="nav-header" id="m_addroot" onclick="addTopMenu();">增加顶级组织</li>
				<li class="nav-header" id="m_del" onclick="deleteMenu();">删除组织</li>
				<li class="nav-header" id="m_edit" onclick="editMenu();">编辑组织</li>
			</ul>
			</div>
			<!-- END PAGE CONTAINER-->
		</div>
	...
	<script>
		jQuery(document).ready(function() {
			$.fn.zTree.init($("#treeOrg"), settingMenu, "");
	...
	function addMenu() {
		...
		if (zTree.getSelectedNodes()[0]) {
			...
			opendg('<%=basePath%>org/form/new?orgid=' + orgid, 600,
					350);
		} else {
			alert("请选中父组织节点");
		}
	...
	function addTopMenu() {
		hideRMenu();
		opendg('<%=basePath%>org/form/new', 600, 350);
	}
	function editMenu() {
		hideRMenu();
		if (zTree.getSelectedNodes()[0]) {
			var orgid = zTree.getSelectedNodes()[0].id;
			opendg('<%=basePath%>org/form/edit?orgid=' + orgid, 600,
					350);
		}
	}
	function viewMenu(){
		...
		if (zTree.getSelectedNodes()[0]) {
			var orgid = zTree.getSelectedNodes()[0].id;
			opendg('<%=basePath%>org/form/view?orgid=' + orgid, 600,
					350);
		}
	}
