...
<!-- END HEAD -->
		...
		<div class="page-content">
			...
			<div class="container-fluid">
				<div class="row-fluid">
					<div class="span12">
						<h3 class="page-title">
							区划同步管理 <small>区划同步</small>
						</h3>
						<ul class="breadcrumb">
							<li><i class="icon-home"></i> <a href="${root}/syn/listSynApp">主页</a>
								<i class="icon-angle-right"></i>
							</li>
							<li><a href="${root}/syn/listApp?opType=syn">同步管理</a></li>
						</ul>
					</div>
				</div>
				<div class="row-fluid">
	...
	function addSyn(){
		var height=500;
		var width=600;
		window.showModalDialog('${root}/syn/gotoSynDivision?appId=${appId}',window,'dialogHeight='+height+',dialogWidth='+width) 
	}
	function updateSyn(orgId){
		window.location.href="${root}/syn/synUpdateDivision?appId=${appId}&orgIds="+orgId;
	}
	function deleteSyn(orgId){
		window.location.href="${root}/syn/synDeleteDivision?appId=${appId}&orgIds="+orgId;
	}
	...
	function gopage(thispage){
		...
		window.location.href="${root}/syn/listSynDivision?thispage="+thispage+"&appId=${appId}";
	}
	function nextpage(){
			if(${thispage}<${totalpage-1}){
				gopage(${thispage+1});
