...
var App = function() {
	...
	var handleFilterInit = function() {
		$("#filter").keyup(function(e) {
			var filter = $(this).val();
			var array = new Array();
			$("li[name='leaf_li']").each(function() {
				// If the list item does not contain the text phrase fade it out
				if (filter != "") {
					if ($(this).text().search(new RegExp(filter, "i")) < 0) {
						$(this).fadeOut();
						$(this).parents('ul.sub-menu').hide();
					...
					$(this).parents('ul.sub-menu').hide();
					$(this).parents('li').removeClass("open");
					$(this).parents('li').find(".arrow").removeClass("open");
				}
			});
			$.each(array, function() {
				$(this).show();
	...
	var handleTheme = function() {
		...
		var setColor = function(color) {
			$('#style_color').attr("href",
					dm_root+"/themes/media/css/" + color + ".css");
		};
		var setBoxColor = function(color) {
			$('.portlet.box').attr("class", "portlet box " + color);
		};
		
		var setLayout = function(layout){
			$('#layoutSetting').val(layout);
		};
		...
		$('li[data="theme"]', panel).click(function() {
			var color1 = $(this).attr("data-style");
			setColor(color1);
			$('li[data="theme"]', panel).each(function() {
				$(this).removeClass("current");
			})
			$(this).addClass("current");
			$.cookie('style_color_ais', color1, {
				expires : 7,
				path : '/',
				secure : false
			});
		...
		$('li[data="box"]', panel).click(function() {
			var color2 = $(this).attr("data-style");
			setBoxColor(color2);
			$('li[data="box"]', panel).each(function() {
				$(this).removeClass("current");
			})
			$(this).addClass("current");
			$.cookie('box_color_ais', color2, {
				expires : 7,
				path : '/',
				secure : false
			});
		...
		var style_color = $.cookie('style_color_ais');
		if (style_color != null) {
			setColor(style_color);
			$('li[data="theme"]', panel).each(function() {
				$(this).removeClass("current");
				var color1 = $(this).attr("data-style");
				if (style_color == color1) {
					$(this).addClass("current");
				}
			})
		}else{
		...
		var box_color = $.cookie('box_color_ais');
		if (box_color != null) {
		...
		var layout = $.cookie('layout_topie');
		if (layout != null) {
			setLayout(layout);
		}else{
			setLayout("1");
	...
	return {
		...
		blockUI : function(el, centerY) {
			...
			el
					.block({
						message : '<img src="'+dm_root+'/themes/media/image/ajax-loading.gif" align="">',
		...
		handleNavInit : function(menu_id) {
			
		},
		handleSideBarColor : function(color) {
			$('#style_color').attr("href",dm_root+
					"/themes/media/resources/" + color + ".css");
		}
